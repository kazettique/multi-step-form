...
interface CommonQuestion extends RawCommonQuestion {}

const rawCommonQuestionList: RawCommonQuestion[] = [
  {
    title: '經濟1',
    options: [
      {
    ...
    options: [
      {
        party: ['DPP'],
        title:
          '2列子曾說過一句意義深遠的話，天地無全功，聖人無全能，萬物無全用。他會這麼說是有理由的。我們要學會站在別人的角度思考。深入的探討經濟，是釐清一切的關鍵。對於一般人來說，經濟究竟象徵著什麼呢？我們要從本質思考，從根本解決問題。老舊的想法已經過時了。問題的關鍵究竟為何？需要考慮周詳經濟的影響及因應對策。若到今天結束時我們都還無法釐清經濟的意義，那想必我們昨天也無法釐清。'
      },
      {
        party: ['KMT', 'TPP'],
        title:
          '2若能夠欣賞到經濟的美，相信我們一定會對經濟改觀。經濟，發生了會如何，不發生又會如何。泰戈爾曾說過，人生雖只有幾十春秋，但它決不是夢一般的幻滅，而是有著無窮可歌可頌的深長意義的; 附和真理，生命便會得到永生。希望大家能從這段話中有所收穫。儘管經濟看似不顯眼，卻佔據了我的腦海。瓦維洛夫曾經提到過，當科學達到某個高峰的時候，它的面前會出現通向新的高峰的廣闊前景，通向進一步發展的嶄新道路。這句話幾乎解讀出了問題的根本。'
      },
      {
        party: ['GTM'],
        title:
          '2動機，可以說是最單純的力量。要想清楚，經濟，到底是一種怎麼樣的存在。老舊的想法已經過時了。帶著這些問題，我們一起來審視經濟。可是，即使是這樣，經濟的出現仍然代表了一定的意義。做好經濟這件事，可以說已經成為了全民運動。經濟的存在，令我無法停止對他的思考。石川磐曾經說過，不需要檢查的管理是理想的管理。這影響了我的價值觀。陳涉相信，燕雀安知鴻鵠之志哉！ 這激勵了我。'
      }
    ]
  }
]

const commonQuestionList = rawCommonQuestionList.map((item, index) => ({
  ...item,
  questionId: index
}))

function genCommonQuestionList(rawList: RawCommonQuestion[]): CommonQuestion[] {
  return rawList.map<CommonQuestion>((item, index) => ({
    ...item,
    questionId123: index
  }))
}

interface RawPartyQuestion {
  answerId: number
  options: { title: string }[]
}

interface PartyQuestion extends RawPartyQuestion {
  questionId: number
}

const rawDppQuestionList: RawPartyQuestion[] = [
  {
    answerId: 0,
    options: [
      {
